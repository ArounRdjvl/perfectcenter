...
import {
    DataGrid,
    GridCellEditCommitParams, GridCellParams,
    GridColDef, GridRenderCellParams,
    GridRowClassNameParams,
    GridToolbar, GridValueGetterParams,
    MuiEvent
...
function getColumns(gridRooms: GridRoom[], setGridRooms: React.Dispatch<React.SetStateAction<GridRoom[]>>): GridColDef[] {
    function genValueGetter(field: string) {
        return (params: GridValueGetterParams<GridRoom>) => {
            return params.row.current?.[field] ?? params.row.default?.[field] ?? "NONE"
        }
    }
...
export function Rooms(props: RoomsProps) {
    ...
    async function init() {
        let rooms = await get<Room[]>('/api/admin/rooms');

        setGridRooms(rooms.map((r) => ({
            id: r.id,
            current: {...r},
            default: r
        })))
    ...
    async function handleVerification() {
        ...
        const roomsEdited = gridRooms.filter(r => r.current && r.default && !isEqual(r.default, r.current))
        ...
        if (roomsAdded.length !== 0 || roomsEdited.length !== 0 || roomsRemoved.length !== 0) {
            setPopup({
                ...
                perform: async () => {
                    let rooms = await post('/api/admin/room/perform', {roomsAdded, roomsEdited, roomsRemoved})
    ...
    function handleCellEdit(params: GridCellEditCommitParams, event: MuiEvent) {
        ...
        // @ts-ignore
        event?.persist?.()
        setGridRooms([...gridRooms.map((room) => {
    ...
    return (<div style={{width: '100%', padding: "1rem", display: "flex", flexDirection: "column", ...props.style}}>
        <div
            style={{flex: "1 0 0"}}
        >
            <Button onClick={() => {
                setGridRooms(old => [...old, {
                    //id: Math.max(...old.map(r => r.id).filter(v => (v != undefined))) + 1,
                    id: Math.max(...old.reduce<number[]>((prev, current) => {
                        if (current.id) {
            ...
            <DataGrid
                ...
                getCellClassName={(params: GridCellParams<GridRoom>) => {
                    if (params.row.default && params.row.current && params.row.current[params.field] !== params.row.default[params.field]) {
        ...
        <Button variant='contained' color='primary' style={{alignSelf: 'flex-end'}} onClick={() => {
            handleVerification()
        }}>apply</Button>
        <Popup
